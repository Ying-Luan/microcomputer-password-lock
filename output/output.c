...
{
  outportb(0x264, _data);
  while (inportb(0x261) & 0x80) // 检查液晶显示是否处于忙状态 BUSY位为DB7, 所以只需要 & 10000000B 来判断是否BUSY，下同
  {
    ;
  }
}
...
{
  outportb(0x260, _data);
  while (inportb(0x261) & 0x80) // 检查液晶显示是否处于忙状态
  {
    ;
  }
}
...
{
 WRComL(x + 0xB8); // 行地址=行序数+行基址
 WRComL(y + 0x40); // 列地址=列序数+列基址
}
// 设置右半屏起始显示行列地址,x:X起始行序数(0-7)，y:Y起始列序数(0-63)
void SETXYR(unsigned char x, unsigned char y)
{
 WRComR(x + 0xB8); // 行地址=行序数+行基址
 WRComR(y + 0x40); // 列地址=列序数+列基址
}
//***************************************
// 显示图形
//***************************************
...
void LCD_INIT()
{
 ...
 WRComL(0xC0); // 设置起始显示行，第0行
 WRComR(0x3e); // 初始化右半屏，关显示
 WRComR(0xC0); // 设置起始显示行，第0行
 LCDClear(); // 清屏
 WRComL(0x3f); // 开显示
 WRComR(0x3f); // 开显示
}
...
unsigned char *address(unsigned char buf);
unsigned int present(unsigned char *buf, unsigned int i);
unsigned char pwd[6] = {'1', '2', '3', '4', '5', '6'}; // 初始密码，定义为字符数组
unsigned int adminPassword = 12345678; // 管理员密码，定义为整型
unsigned char tt; // 用于main函数，存放用户按下的键的键值
// 延时子程序
void delay(unsigned int ms) // 1000
{
 unsigned int i;
 while (ms--) // 1000
 {
...
// 判断是否有键被按下
unsigned char AllKey()
{
 unsigned char i; // i是标志，i=0表示没有键被按下，否则表示有键被按下
 outportb(0x271, 0x0); // B口送出信号，同时检测所有按键
 i = (~inportb(0x272) & 0x3); // 读C口数据，若PC1(KL2) PC0(KL1)为1 1，说明没有键被按下，返回0，一旦有某个键被按下，则返回非0
 return i;
}
// 读取键盘输入
unsigned char key()
{
 unsigned char i, j, keyResult;
 unsigned char bNoKey = 1; // 初始化没有键被按下为1
 while (bNoKey)
 {
  ...
  do
  {
   outportb(0x271, i); // 扫描某列
   j = ~inportb(0x272); // 读取KL2 KL1的值
...
unsigned char shift()
{
 unsigned char currChar = key();
 while (currChar > 0x09) // 若按下的键值大于9，则不断要求用户重新输入
  currChar = key();
 return currChar + (unsigned char)'a'; // 在'a'的ASCII码基础上进行0-9的偏移，得到不同字母('a'-'j')的ASCII码
}
// 绿灯亮10s
void greenl()
{
 outportb(0x270, 0xfe); // 绿灯1111 1110（参考硬件F4模块电路原理图）
 delay(10000);
}
// 红灯亮5s
void redl()
{
 outportb(0x270, 0xfb); // 红灯1111 1011
 delay(5000);
}
// 黄灯闪烁5s及蜂鸣器发出警报声
void yellowl()
{
 ...
 for (i = 0; i < 10; i++)
 {
  outportb(0x270, 0xfd); // 黄灯1111 1101
  outportb(0x273, 0x0e); // 写控制字0000 1110————C口位操作：PC7复位，蜂鸣器响
  delay(250);
  outportb(0x270, 0xff); // 黄灯熄灭
  outportb(0x273, 0x0f); // 写控制字0000 1111————C口位操作：PC7置位，蜂鸣器停
  delay(250);
 }
}
// 全熄灭
void blackl()
{
 outportb(0x270, 0xff); // 全灭 1111 1111
}
// -----------------------------------------下面是功能函数-----------------------------------------
// 输入密码并与原密码进行比较，如果正确则返回1，否则返回0
unsigned int checkPwd()
{
 unsigned int i;
 unsigned char buf[6];
 //	u16 inputPassword = 0;
...
void changePwd()
{
 ...
 for (i = 0; i < 6; i++)
 {
  ...
  if (buf[i] == 0x0e) // 如果按下shift键
   ...
   buf[i] += (unsigned char)'0'; // 数字变量要转成unsigned char类型，即转成ASCII码型数字，方便显示
  LCD_INIT();
  switch (i)
  {
  case 0:
...
unsigned int adminOption()
{
 unsigned char i;
 unsigned int buf[8];
 while (1)
 {
  unsigned int inputPassword = 0; // ！！！！！！！！！初始化！！！！！！！！！
...
void main()
{
 outportb(0x273, 0x81); // 写控制字1000 0001————A口、B口方式0，输出；C口低4位：输入
 outportb(0x273, 0x0f); // 写控制字0000 1111————C口位操作：将PC7置位，控制蜂鸣器
