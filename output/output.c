 ...
 */
void show4Characters(unsigned char *char1, unsigned char *char2, unsigned char *char3, unsigned char *char4)
{
...
// 显示*
// count表示要显示多少个*
void showStars(unsigned int count)
{
  unsigned int i;
  for (i = 0; i < count; i++)
  {
...
unsigned char adminPwd[8] = {'1', '2', '3', '4', '5', '6', '7', '8'}; // 管理员密码，定义为字符数组
// -----------------------------------------底层工具函数-----------------------------------------
...
unsigned int checkPwd()
{
 ...
 for (i = 0; i < 6; i++)
 {
  ...
  if (buf[i] == 0x0e)
  {
   unsigned char shiftResult = shift();
   if (shiftResult == 0)
    i = present(buf, i);
   else
    buf[i] = shiftResult;
...
void changePwd()
{
 ...
 for (i = 0; i < 6; i++)
 {
  ...
  if (buf[i] == 0x0e) // 如果按下shift键
   ...
   buf[i] += (unsigned char)'0'; // 数字变量要转成u8类型，即转成ASCII码型数字，方便显示
  LCD_INIT();
  showStars(i + 1);
 }
 while (1)
